...
export class HarvestApi {
    ...
    public async getMyProjects(): Promise<HarvestProject[]> {
        ...
        const response = await fetch(`${this.baseUrlV2}/users/me/project_assignments`, { headers: this.createHeaders() });
    ...
    public async getTimeEntries(date: string): Promise<HarvestTimeEntry[]> {
        ...
        const response = await fetch(`${this.baseUrlV2}/time_entries?user_id=${userId}&from=${date}&to=${date}`, {
    ...
    public async startTimeEntry(
        ...
        const response = await fetch(`${this.baseUrlV2}/time_entries`, {
            method: "POST",
            headers: this.createHeaders(),
    ...
    private async getMyId(): Promise<number> {
        const response = await fetch(`${this.baseUrlV2}/users/me`, { headers: this.createHeaders() });
    ...
    private async patchOperation(directive: string, entryId: number, updatedNotes = ""): Promise<boolean> {
        const url = directive
            ? `${this.baseUrlV2}/time_entries/${entryId}/${directive}`
            : `${this.baseUrlV2}/time_entries/${entryId}`;
